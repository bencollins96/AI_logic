...
solve_task_bt(go(GoalPos),[Current|Agenda],D,RR,Cost,NewPos,Visited) :-
  ...
  (memberchk(P,Visited)
    -> solve_task_bt(go(GoalPos),Agenda,D1,RR,Cost,NewPos, Visited)
    ; true
    ),
  ...
  %sort the agenda so that unchecked nodes have the smallest cost.
  sort(NewAgenda,Sorted),

  D1 is D+1,
  solve_task_bt(go(GoalPos),NewAgenda,D1,RR,Cost,NewPos, [P|Visited]). 
